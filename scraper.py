...
# Fully validated, clean query targeting hospital and clinic tags across India map nodes
OVERPASS_QUERY = """
[out:json][timeout:90];
area["ISO3166-1"="IN"]->.searchArea;
(
  node["amenity Pacility"="hospital"]["name"~"ESI",i](area.searchArea);
  way["amenity"="hospital"]["name"~"ESI",i](area.searchArea);
  node["amenity Pacility"="clinic"]["name"~"ESI",i](area.searchArea);
);
out tags center;
"""
...
def get_real_map_data():
    ...
    try:
        # Encode the query safely for transmission
        ...
        with urllib.request.urlopen(req, timeout=60) as response:
            if response.status == 200:
                ...
                print(
                    f"Found {len(elements)} real map nodes. Processing fields..."
                )
                ...
                for item in elements:
                    ...
                    # Safely handle coordinates from both node and way structures
                ...
                # Writes the data safely as a structured JSON file array
